...
Invitación web tipo cassette para la fiesta **MEMORIAS**, el **lunes 24 de agosto** desde las **21:00 hs** en **Coraceros 3723, Capurro, Montevideo**.

Sitio estático hecho 100% en **HTML + CSS + JS moderno** (sin frameworks, sin build), mobile-first, con **playlist de audio** (play/pausa, anterior/siguiente, mute), SEO básico y vista previa optimizada para compartir por WhatsApp.

🔗 **Sitio publicado:** https://bailemos24.netlify.app/
...
├── musica/                 # Playlist (los archivos se cargan por nombre exacto)
│   ├── devo.mp3
│   ├── Los-Tipitos-Campanas.mp3
│   ├── MIRANDA - Don (Don Pich! 12'' Mix).mp3
│   └── Soda Stereo - Gustavo Cerati & Cafe Tacuba - Juegos de SeducciU_n (Vivo).mp3
├── cassete.svg             # Ícono del cassette — favicon principal (SVG)
├── og-image.png            # Imagen 1200x630 para la vista previa de WhatsApp / redes
├── og-cassette.svg         # Versión vectorial del OG image
...
Todos los archivos deben quedar **en la misma estructura**, sin renombrarlos — `index.html` los referencia por nombre exacto y ruta relativa (ojo: los nombres de los MP3 contienen espacios, comillas y caracteres especiales; no los cambies).
...
2. *Settings → Pages → Deploy from branch* → rama `main`/`master`, carpeta `/root`
...
| Canciones de la playlist | Array `const playlist = [...]` en el `<script>` final |

---

## 🔊 Cómo funciona el audio

- Tocar el **cassette**, cualquier parte del `<main id="V">`, o el botón central reproduce la playlist en loop.
- **⏮ / ⏭**: canción anterior / siguiente (circular). Al terminar una canción pasa automáticamente a la próxima.
- **🔊 / 🔇**: botón de silenciar/reactivar.
- El botón central muestra qué tema está sonando: `🎵 <nombre del track>`.
...
- Para agregar o quitar temas: editá el array `playlist` en el JS (objeto `{ file, name }`) y dejá el MP3 dentro de `musica/`.

**Nota de tamaño**: los MP3 suman varios MB. Si el sitio va a viajar mucho por redes lentas, conviene comprimirlos a ~96 kbps antes de publicar (con Audacity, ffmpeg, o cualquier conversor online) — el tamaño no afecta la carga de la página en sí porque el audio tiene `preload="none"`, pero sí la espera antes de que arranque a sonar cada tema la primera vez.

---

## ✅ Checklist antes de mandar el link

- [x] Reemplazar la URL placeholder por la real — hecho: `https://bailemos24.netlify.app/`
- [ ] Probar el link en el propio WhatsApp (mandártelo a vos mismo) y confirmar que se ve la imagen de vista previa
- [ ] Probar en un celular real que el audio arranca al tocar y que prev/next funcionan
...
1. Comprimir los MP3 a menor bitrate para una primera reproducción más rápida
2. Agregar contador regresivo hasta el 24 de agosto
3. Versión con formulario de confirmación propio (en vez de depender solo del link de WhatsApp) si la lista de invitados crece
